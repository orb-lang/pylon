...
But the normal migration is just a series of executed statements, and it may
be cleaner to allow for a migration to be an array\-table of such statements\.

Migrations are definitely a case where factoring out the common steps in
booting and migrating a database into a helper module is going to be worth it\.

We only migrate one database currently, but I'm about to add a migration to
`bridge.modules` and start a third database for `orb` metadata, so this will
happen \(relatively\) soon\.

Note that `user_version` must start with 2, because SQLite uses 0 for false
and 1 for true, and we cast those internally to booleans\.
