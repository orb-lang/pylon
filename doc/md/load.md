...
This one we might export\.

Takes a string and loads it using a clean fenv, validating table\-ness if
...
local function parse_list(str)
   ...
   if not list then
      list = tonumber(str)
      if not list then
         return nil, "couldn't make a list from " .. str
      end
   end
...
helm_c :group( "Sessions",
   helm_c
      : option "--new-session" "-n"
   ...
   helm_c
      : option "--start" "-s"
         : description "Start the repl with session, given name or number. \z
                        Defaults to the latest session of the project."
   ...
   helm_c
      : option "--session" "-S"
         : description "Edit a session of the given name or number."
         : convert(parse_session_identifier)
         : argname "<s>"
         : args(1)
)
...
helm_c
   : flag "-r --rerun"
   : description "Start helm and execute all lines from the last run."
   : target "restart" -- #todo remove

helm_c
   : flag "-R --review"
   : description "Edit the last run then restart. May also take a session \z
                  with -s."
   : target "run" -- #todo remove

helm_c
   : flag "-l --listen"
   : description "Open with a listener, which compiles files on save and \z
...
```sql
SELECT active, name
FROM voltron WHERE (name, voltron_id) IN
(SELECT name, MAX(voltron_id)
   FROM voltron
   GROUP BY name)
;
```
